...
async function verify() {
  ...
  const sourceCode = fs.readFileSync('CipherLifeVault_no_bom.sol', 'utf8');
  ...
  try {
    console.log("Submitting verification request for", contractAddress);
    ...
    if (response.data.status === '1') {
      ...
      console.log("Verification failed to initiate:", response.data.result);
    }
  } catch (error) {
